...
export function LocationSearch({
    ...
    void import('leaflet').then((leafletModule) => {
      ...
      map.on('click', async (event: any) => {
        try {
          ...
          const selectedPlace = await reverseGeocodeOpenStreetMap(event.latlng.lat, event.latlng.lng)
          ...
          setStatusMessage(
            error instanceof Error ? error.message : 'Không thể đọc địa chỉ từ vị trí đã chọn.'
          )
        }
      })
  ...
  useEffect(() => {
    ...
    if (trimmedQuery.length < 3) {
      ...
      setStatusMessage(trimmedQuery.length === 0 ? '' : 'Nhập ít nhất 3 ký tự để hiện gợi ý địa chỉ.')
      return
    }
    ...
    const timer = window.setTimeout(() => {
      searchOpenStreetMap(trimmedQuery)
        ...
        .catch((error) => {
          ...
          setSearchResults([])
          setStatusMessage(
            error instanceof Error ? error.message : 'Không thể tìm địa chỉ từ OpenStreetMap.'
          )
        })
        .finally(() => {
          if (isActive) {
  ...
  return (
    <div className="sm:col-span-2 rounded-3xl border border-[var(--border)] bg-white p-4">
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-[var(--border)] bg-[var(--panel)] py-3 pl-11 pr-4 text-sm outline-none"
            placeholder={placeholder}
          />
          {isSearching ? (
            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-medium text-[var(--muted)]">
              Đang gợi ý...
            </span>
          ) : null}
        </div>
        ...
        <div className="mt-3 grid gap-2">
          {searchResults.map((place) => (
            <button
              ...
              className="flex items-start gap-3 rounded-2xl border border-[var(--border)] bg-[var(--panel)] px-4 py-3 text-left transition hover:border-[var(--primary)]"
            >
              <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[var(--primary)]" />
              <span className="text-sm leading-6 text-[var(--foreground)]">{place.formattedAddress}</span>
            </button>
          ))}
        </div>
      ) : null}

      <div ref={mapContainerRef} className="mt-4 h-64 overflow-hidden rounded-2xl border border-[var(--border)]" />

      <p className="mt-2 text-xs leading-5 text-[var(--muted)]">
        Gõ địa chỉ để nhận gợi ý tự động. Bạn cũng có thể click trực tiếp lên bản đồ để lấy vị trí gần đúng.
      </p>

      {statusMessage ? <p className="mt-2 text-xs font-medium text-[var(--foreground)]">{statusMessage}</p> : null}
    </div>
  )
}
