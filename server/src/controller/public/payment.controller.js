...
async function updatePaymentState({ orderId, provider, status, transactionCode }) {
    ...
    if (!payment) {
        throw new NotFoundError('Kh?ng t?m th?y giao d?ch thanh to?n');
    }
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (nextPaymentStatus === 'success') {
            const cart = await tx.carts.findUnique({
                where: { user_id: Number(payment.orders?.users?.id) },
                select: { id: true },
            });
    ...
    async paymentCallback(req, res) {
        ...
        const redirectUrl =
            status === 'success'
                ...
                : `${getClientBaseUrl()}/checkout/failed?orderId=${orderId}&provider=${provider}`;
